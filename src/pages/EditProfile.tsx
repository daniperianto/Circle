...
import notification from "../libs/notification.ts";
import {ToastContainer} from "react-toastify";
import {useDispatch} from "react-redux";
import {AUTH_UPDATE} from "../store/rootReducer.ts";
...
export default function EditProfile() {
    ...
    async function handleSubmit() {
        try {
            const response = await API.put(`/user/${userId}`, form)

            if(response.data.message == "updated success"){
                notification.success("Success")
                dispatch(AUTH_UPDATE(response.data.data))
            }
        } catch (error) {
            console.log(error)
        }
